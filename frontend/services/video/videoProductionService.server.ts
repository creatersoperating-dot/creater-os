...
interface VisualAssetRow {
  ...
  scene_plan_id: string; scene_id: string; scene_number: number; format: "svg" | null;
  mime_type: "image/svg+xml" | null; width: number | null; height: number | null;
...
function sceneStoragePath(userId: string, brandId: string, projectId: string, generationId: string, sceneNumber: number): string {
  return `${userId}/${brandId}/${projectId}/${generationId}/scenes/${sceneNumber}.svg`;
}
...
export class VideoProductionService {
  ...
  async generate(brandIdValue: string, projectIdValue: string, operationIdValue: string, renderer: ConfiguredVideoRenderer, retryGenerationId?: string, incomingSignal?: AbortSignal): Promise<VideoLifecycleResponse> {
    const brandId = required(brandIdValue, "brandId"); const projectId = required(projectIdValue, "projectId");
    const operationId = requiredUuid(operationIdValue, "operationId");
    const { adapter, model, timeoutMs, activeLeaseMs, heartbeatMs } = renderer;
    ...
    try {
      ...
      const durationEligibility = evaluateVideoDurationEligibility(
        plannedDurationMs,
        adapter.descriptor.capabilities.maximumDurationMs,
      );
      ...
      if (durationEligibility.violation === "provider_limit_exceeded") {
        throw new VideoProductionError("provider_duration_unsupported", `${adapter.descriptor.label} supports videos up to ${Math.floor(durationEligibility.maximumDurationMs / 60_000)} minutes.`, false, 422);
      }
      ...
      const { data: reservedData, error: reserveError } = await this.db.rpc("claim_video_generation_operation", {
        ...
        p_source_scene_plan_id: plan.id, p_provider: adapter.descriptor.id, p_model: model,
      ...
      const { data: claimedData, error: claimError } = await this.db.rpc("start_video_generation_attempt", {
        ...
        p_provider: adapter.descriptor.id, p_model: model,
      });
      throwIfAborted(signal);
      ...
      const assets = await adapter.generateVisualAssets(renderRequest);
      assertHeartbeat();
      if (assets.length !== plan.scenes.length) throw new VideoProviderError("invalid_asset_set", "The video renderer returned an incomplete scene visual set.");
      ...
      for (const asset of assets) {
        ...
        if (!scene || providerSceneIds.has(asset.sceneId) || providerSceneNumbers.has(asset.sceneNumber)) {
          throw new VideoProviderError("invalid_asset_set", "The video renderer returned an invalid scene visual set.");
        }
      ...
      for (const asset of assets) {
        ...
        assertHeartbeat();
        const scene = plan.scenes.find((item) => item.id === asset.sceneId && item.sceneNumber === asset.sceneNumber);
        if (!scene || asset.bytes.byteLength === 0 || asset.format !== "svg" || asset.mimeType !== "image/svg+xml" || asset.width <= 0 || asset.height <= 0) throw new VideoProviderError("invalid_asset", "The video renderer returned an invalid scene visual.");
        const path = sceneStoragePath(this.userId, brandId, projectId, generation.id, asset.sceneNumber);
        ...
        if (assetRow?.status === "ready") {
          const metadataMatches = assetRow.scene_plan_id === plan.id && assetRow.source_scene_plan_version === plan.version && assetRow.scene_id === asset.sceneId && assetRow.source_scene_sha256 === sourceSceneSha && assetRow.storage_path === path && assetRow.file_size_bytes === asset.bytes.byteLength && assetRow.content_sha256 === contentSha && assetRow.mime_type === asset.mimeType && assetRow.width === asset.width && assetRow.height === asset.height;
          if (metadataMatches && await this.objectContentMatches(path, asset.bytes.byteLength, asset.mimeType, contentSha, signal)) continue;
          if (await this.storageEntry(path, signal)) throw new VideoProductionError("asset_integrity_failed", "A finalized scene visual failed integrity validation. Start a new generation.", false, 409);
        ...
        if (!assetRow) {
          ...
          const { error: cleanupError } = await this.db.storage.from(BUCKET).remove([path]);
          throwIfAborted(signal);
          if (cleanupError) throw new VideoProductionError("cleanup_failed", "A failed scene visual could not be prepared for retry.", true, 500);
        }
        const { data: generating, error: generatingError } = await this.db.from("video_visual_assets").update({ status: "generating", scene_plan_id: plan.id, scene_id: asset.sceneId, source_scene_title: scene.title, source_narration_text: scene.narrationText, source_visual_prompt: scene.visualPrompt, source_visual_type: scene.visualType, source_duration_ms: scene.durationMs, source_transition: scene.transition, source_scene_sha256: sourceSceneSha, source_scene_plan_version: plan.version, provider: adapter.descriptor.id, model, failure_code: null, failure_message: null, cleanup_pending: false }).eq("user_id", this.userId).eq("id", assetRow.id).in("status", ["queued", "failed", "generating", "uploading"]).select("id").maybeSingle();
        throwIfAborted(signal);
        ...
        try {
          ...
          if (!await this.objectContentMatches(path, asset.bytes.byteLength, asset.mimeType, contentSha, signal)) throw new VideoProductionError("asset_upload_invalid", "A scene visual upload could not be verified.", true, 500);
          const { data: readyAsset, error: readyAssetError } = await this.db.from("video_visual_assets").update({ status: "ready", format: asset.format, mime_type: asset.mimeType, width: asset.width, height: asset.height, file_size_bytes: asset.bytes.byteLength, content_sha256: contentSha, cleanup_pending: false }).eq("user_id", this.userId).eq("id", assetRow.id).eq("status", "uploading").select("id").maybeSingle();
          throwIfAborted(signal);
          if (readyAssetError || !readyAsset) throw new VideoProductionError("asset_persistence_failed", "A scene visual ready state could not be saved.", true, 500);
          await heartbeat();
          assertHeartbeat();
        } catch (error) {
          const failure = safeFailure(error);
          await this.markAssetFailed(assetRow.id, failure.code, failure.message);
          let cleanupPending = false;
          try { const cleanup = await this.db.storage.from(BUCKET).remove([path]); cleanupPending = Boolean(cleanup.error); }
          catch { cleanupPending = true; }
          if (cleanupPending) await this.markAssetFailed(assetRow.id, failure.code, failure.message, true);
          throw error;
        }
      }
      ...
      const failure = safeFailure(error);
      safeLog("[CreatorOS video generation failure]", { stage: "video_lifecycle", provider: adapter.descriptor.id, configuredModel: model, errorName: error instanceof Error ? error.name.slice(0, 100) : "UnknownError", failureCode: failure.code, sanitizedMessage: failure.message, retryable: failure.retryable });
